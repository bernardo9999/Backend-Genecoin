...
import { NotasController, NotasTransientInput, NotasPrivate } from '../src';
...
describe('Notas', () => {
  ...
  //-----------------------------------------------------
  it('Armazenar dados privados', async () => {
    ...
    console.log('Enviando tx com transient:', transientInput.toJSON());
    await notasCtrl
      .$config({ transient: { nota: transientInput.toJSON() } })
      .initNotasOrg1();

    const notasPrivateRaw = await adapter.stub.getPrivateData('collectionNotasPrivateOrg1', transientInput.id);
    const notasPrivate = new NotasPrivate(JSON.parse(notasPrivateRaw.toString('utf8')));
  ...
  it('Armazenar dados privados', async () => {
    ...
    const notasPrivateRaw = await adapter.stub.getPrivateData('collectionNotasPrivateOrg2', transientInput.id);
    const notasPrivate = new NotasPrivate(JSON.parse(notasPrivateRaw.toString('utf8')));
...
//   const nota = await notasCtrl.$query().getOneNotasPrivateOrg2('48190318866111000493550010000114001068302221');
//   console.log('Got private data:', nota);
